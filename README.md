...
- Grant your terminal **App Management** access (System Settings → Privacy & Security → App Management) if `brew bundle` failed on a cask with `chgrp: … Operation not permitted`. macOS blocks one app from modifying another's bundle, and `sudo` does not bypass it, so adopting an app that was installed by hand fails every time. Deleting the app and letting Homebrew install it fresh is the one-off way out; the permission is what keeps future `brew upgrade --cask` working.
